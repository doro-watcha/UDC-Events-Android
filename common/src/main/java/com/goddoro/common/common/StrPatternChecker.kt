...
object StrPatternChecker {
    ...
    private const val str_name_pattern = "^[a-zA-Z0-9.]{6,24}$"
    private val NameMatcher = Pattern.compile(str_name_pattern)

    //    private static final String str_user_name_pattern = "^(?=.*[a-zA-Z0-9.]){6,24}([a-zA-Z0-9]+[.]{0,1}[a-zA-Z0-9]+)$";
    private const val str_user_name_pattern = "^(?=.{6,24}$)([a-zA-Z0-9]+[.]{0,1}[a-zA-Z0-9]+)$"
    ...
    fun NameTypeOk(_contents: String?): Boolean {
        return NameMatcher.matcher(_contents).matches()
    }
